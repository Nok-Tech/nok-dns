...
class CloudflareClient:
    def __init__(self, token: str):
        self.token = token
        self.http = httpx.Client(timeout=10.0, headers={"Authorization": f"Bearer {token}"})
    ...
    def find_record_id(self, zone_id: str, record_name: str, record_type: str) -> Optional[str]:
        response = self.http.get(
            f"{self.base_url}/zones/{zone_id}/dns_records", params={"type": record_type, "name": record_name}
        )
        response.raise_for_status()
